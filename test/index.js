...
after(async () => {
  if (typeof server.stop === 'function') return server.stop()
})
...
describe('Error Messages', () => {
  ...
  describe('Options', () => {
    ...
    it('should not strip quotes by default', async () => {
      const res = await server.inject({ method: 'POST', url: path + '/no-options', payload: {} })
    ...
    it('should strip quotes when stripQuotes option is true', async () => {
      const res = await server.inject({ method: 'POST', url: path + '/no-quotes', payload: {} })
